...
func generateDelivery() *Delivery {
	return &Delivery{
		Name:    generateRandomString(1, rand.Intn(20)),
...
func generatePayment() *Payment {
	return &Payment{
		...
		Amount:        uint(rand.Uint32()),
		PaymentDT:     time.Now().Add(time.Duration(rand.Intn(1000)) * time.Minute),
		Bank:          generateRandomString(3, rand.Intn(15)),
		DeliveryCost:  uint(rand.Uint32()),
		GoodsTotal:    uint(rand.Uint32()),
		CustomFee:     uint(rand.Uint32()),
	}
}
...
func generateOrderItem() *Item {
	return &Item{
		...
		Name:           generateRandomString(3, rand.Intn(30)),
		Sale:           int8(rand.Intn(100)),
		Size:           generateRandomString(2, rand.Intn(5)),
		TotalPrice:     uint(rand.Uint32()),
...
func generateOrder() *Order {
	...
	return &Order{
		...
		TrackNumber:       generateRandomString(1, rand.Intn(15)),
		Entry:             generateRandomString(3, rand.Intn(15)),
		Delivery:          *generateDelivery(),
		Payment:           *generatePayment(),
		Items:             items,
		Locale:            generateRandomString(3, rand.Intn(2)),
		InternalSignature: generateRandomString(1, rand.Intn(5)),
		CustomerID:        generateRandomString(3, rand.Intn(10)),
